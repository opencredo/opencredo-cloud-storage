...
public class ToStringTransformer {
     ...
     * @param template
     */
    public ToStringTransformer(AwsOperations template) {
    ...
    public Message<String> transform(Message<Map<String, Object>> message) {
        ...
        Assert.notNull(payload.get("bucketName"), "Bucket name must be specified in the header");
        Assert.notNull(payload.get("key"), "Bucket name must be specified in the header");

        String transformedString = template.receiveAsString(payload.get("bucketName").toString(), payload.get("key")
                .toString());
