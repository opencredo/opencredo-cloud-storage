 ...
 */
@Ignore ("Need to review those tests - Tomas")
public class MainAppTest {
	ReadingMessageSource inboundAdapter;
	ToStringTransformer toStringTransformer;
	WritingMessageHandler outboundAdapter;
	...
	public void init() throws IOException, S3ServiceException{
		template = new S3Template(awsCredentials, bucketName);
		
		inboundAdapter = new ReadingMessageSource(template, bucketName);
		toStringTransformer = new ToStringTransformer(template);
		outboundAdapter = new WritingMessageHandler(template, bucketName);
	}
	...
	public void testFileContentsReadFromS3AndUploadedToS3AsString() throws IOException, S3ServiceException {
		...
		Message<Map<String, Object>> receivedMessage = inboundAdapter.receive();
		
		Message<String> transformedMessage = toStringTransformer.transform(receivedMessage);
		...
		outboundAdapter.handleMessage(builder.build());

		String receiveAsString = template.receiveAsString(key);
		assertEquals(payload, receiveAsString);
	}
